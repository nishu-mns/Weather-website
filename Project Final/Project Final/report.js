...
.then(data => {
    ...
    for(i = 0; i<6; i++){
        document.getElementById("day" + (i+1)).innerHTML = "Temperature " + Number(data.list[i].main.temp - 273.15).toFixed(1)+ "°";
    ...
    for(i = 0; i<6; i++){
        document.getElementById("wind" + (i+1)).innerHTML = "Wind Speed:  " + Number(data.list[i].wind.speed)+"km/h";
    }
